...
export class GlyphTool extends AbstractTool {
    name = 'glyph';
    title = 'Glyph';
    ...
    createLayer(): AbstractLayer {
        ...
        return new GlyphLayer(session.getPlatformFeatures(), renderer, this.editor.font, session.state.platform);
    }

    isSupported(platform: string): boolean {
        return platform === 'u8g2';
    }

    onStopEdit(layer: GlyphLayer, position: Point, originalEvent: MouseEvent | TouchEvent): void {
    ...
    onActivate(): void {
        ...
        // Center the glyph on the display.
